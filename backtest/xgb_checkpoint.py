...
def _checkpoint_row(monthly, daily, calendar, targets, group_cols, target_month, checkpoint, key, min_train_months):
    ...
    if row is None:
        return None
    cur = daily[(daily["invoice_date"] >= target_month) & (daily["invoice_date"] <= checkpoint_date)]
    for col, value in zip(group_cols, key):
        cur = cur[cur[col] == value]
    mtd_value = float(cur["sellin_value"].sum()) if not cur.empty else 0.0
    month_calendar = calendar[(calendar["date"] >= target_month) & (calendar["date"] < target_month + pd.offsets.MonthBegin(1))]
    total_wd = int(month_calendar["is_working_day"].astype(bool).sum())
    elapsed_wd = int(month_calendar.loc[month_calendar["date"] <= checkpoint_date, "is_working_day"].astype(bool).sum())
    target = targets
    for col, value in zip(group_cols, key):
...
def _build_training_frame(monthly, daily, calendar, targets, group_cols, target_month, checkpoint, min_train_months):
    ...
    for historical_target in prior_months:
        ...
        for key in keys:
            row = _checkpoint_row(monthly, daily, calendar, targets, group_cols, historical_target, checkpoint, _as_key(key), min_train_months)
...
def _build_all_checkpoint_training_frames(monthly, daily, calendar, targets, group_cols, checkpoints, min_train_months):
    ...
    return {int(cp): _build_training_frame(monthly, daily, calendar, targets, group_cols, snapshot_end, int(cp), min_train_months) for cp in checkpoints}


def rolling_xgb_checkpoint_backtest(monthly_history, daily_history, calendar, group_cols, checkpoints, min_train_months=24, targets=None, return_predictions=False):
    ...
    if targets is None:
        ...
        targets = targets.copy(); targets["periode"] = pd.to_datetime(targets["periode"])
    ...
    checkpoint_frames = _build_all_checkpoint_training_frames(monthly, daily, calendar, targets, group_cols, checkpoints, min_train_months)
    ...
    for target_month in sorted(monthly["periode"].drop_duplicates()):
        for cp in checkpoints:
            ...
            for key in keys:
                key = _as_key(key)
                row = _checkpoint_row(monthly, daily, calendar, targets, group_cols, target_month, cp, key, min_train_months)
                ...
                if pred is not None and np.isfinite(pred):
                    pair_store[key][cp].append((pd.Timestamp(target_month), float(row["actual"]), float(pred)))

    results = []
    for key, cp_pairs in pair_store.items():
        out = dict(zip(group_cols, key))
        all_pairs = [(a, p) for pairs in cp_pairs.values() for _, a, p in pairs]
        for cp in checkpoints:
            pairs = [(a, p) for _, a, p in cp_pairs[cp]]
            if pairs:
                y, p = zip(*pairs)
                out[f"xgboost_wd{cp}_wape"] = wape(y, p); out[f"xgboost_wd{cp}_mae"] = mae(y, p); out[f"xgboost_wd{cp}_rmse"] = rmse(y, p); out[f"xgboost_wd{cp}_bias"] = bias(y, p); out[f"xgboost_wd{cp}_bias_pct"] = bias_pct(y, p)
            else:
                for metric in ("wape", "mae", "rmse", "bias", "bias_pct"): out[f"xgboost_wd{cp}_{metric}"] = np.nan
            out[f"xgboost_wd{cp}_observations"] = len(pairs)
            q10, q90, count = _residual_stats(pairs)
            out[f"xgboost_wd{cp}_residual_q10"] = q10; out[f"xgboost_wd{cp}_residual_q90"] = q90; out[f"xgboost_wd{cp}_residual_observations"] = count
        if all_pairs:
            y, p = zip(*all_pairs)
    ...
    return (result_df, pair_store) if return_predictions else result_df

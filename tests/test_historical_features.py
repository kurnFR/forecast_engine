...
def test_momentum_feature_does_not_use_current_target():
    ...
    assert row_a["mom_growth"] == pytest.approx(row_b["mom_growth"])
    assert row_a["lag_1"] == pytest.approx(row_b["lag_1"])
    assert row_a["rolling_mean_3"] == pytest.approx(row_b["rolling_mean_3"])
...
def test_momentum_is_previous_month_growth():
    ...
    assert march["mom_growth"] == pytest.approx(0.2)
